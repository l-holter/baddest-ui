...
export function EmailInput({ email, emailProvider, onEmailChange, onEmailProviderChange, errors }: EmailInput) {
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    if (isAlphanumeric(event.key)) {
      ...
      onEmailChange(email.slice(0, -1));
    }
  }

  const handleSelectEmailProvider = (value: string) => {
    console.log(value);
    onEmailProviderChange(value);
  }

  const handleAcceptInput = () => {
    onEmailChange(`${email}${currentChar}`);
    setCurrentChar("");
    setShowDialog(false);
  }
  ...
  return (
    <div>
      ...
      <div className="flex flex-row justify-between items-center">
        ...
        <select className="max-width-10" value={emailProvider} onChange={e => handleSelectEmailProvider(e.currentTarget.value)}>
          {EmailProviders.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
      ...
      {showDialog && (
        <div className="dialog">
          <div className="dialog-content">
            <p>Are you sure you want to use "{currentChar}"?</p>
            <div className="flex flex-row justify-between">
              <button onClick={closeDialog}>No</button>
              <button onClick={() => handleAcceptInput()}>Accept input</button>
            </div>
...
export const validateEmail = (email: string, emailProvider: string): FormErrors => {
  const newErrors: FormErrors = {};

  if (email) {
    newErrors.email = 'Email is required';
  } else if (!/\S+@\S+\.\S+/.test(`${email}@${emailProvider}`)) {
